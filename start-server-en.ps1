...
# Search for Node.js in standard paths
$possiblePaths = @(
    "$env:ProgramFiles\nodejs\node.exe",
...
if (-not $nodePath) {
    Write-Host "[ERROR] Node.js not found in standard paths" -ForegroundColor Red
    Write-Host ""
...
try {
    ...
    if (Test-Path $tsxPath) {
        ...
        Write-Host "[ERROR] tsx not found! Run: npm install --legacy-peer-deps" -ForegroundColor Red
        pause
        exit 1
    }
} catch {
