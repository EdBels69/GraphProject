...
export class CrossrefSource implements ISearchSource {
    name = 'crossref'

    async search(query: string, options?: SearchSourceOptions): Promise<SearchResult[]> {
        try {
            const rows = options?.maxResults || 20
            const filter = options?.fromDate ? `filter=from-pub-date:${options.fromDate},until-pub-date:${options.toDate || new Date().toISOString().split('T')[0]}` : ''

            const url = `https://api.crossref.org/works?query=${encodeURIComponent(query)}&rows=${rows}&${filter}`
            ...
            logger.error('CrossrefSource', `Search failed for query: ${query}`, { error })
            return []
        }
    }
}
